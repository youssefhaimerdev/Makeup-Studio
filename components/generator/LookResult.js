...
function StepRow({ step, index, isExpanded, onToggle, palette }) {
  ...
  return (
    <div onClick={onToggle}
      className={`border rounded-xl px-4 py-3.5 cursor-pointer transition-all duration-150 hover:shadow-sm ${hasProduct ? "border-nude-100 hover:border-rose-200" : "border-rose-100"}`}
      style={{ background:"var(--bg-card)" }}>
      <div className="flex items-center gap-3">
        <div className="w-7 h-7 rounded-full text-xs font-bold font-sans flex items-center justify-center shrink-0"
          style={{ background:dotBg, color:"#fff" }}>{index+1}</div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-1">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={()=>setTutorialOpen(true)}
            className="flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold font-sans cursor-pointer border transition-all"
      ...
      {analysis.length>0 && (
        <div className="border rounded-2xl overflow-hidden" style={{ background:"var(--bg-card)", borderColor:"var(--border)" }}>
          <button onClick={()=>setShowAnalysis(v=>!v)}
            className="w-full flex items-center justify-between px-5 py-4 border-none cursor-pointer font-sans"
